...
The cache is intentionally bypassed during two PT time windows — **2:00am–7:00am** and **1:00pm–4:00pm** — when the majority of newsletters from approved senders are observed to arrive. During these windows `GET /api/digest` returns `null`, prompting the client to trigger a fresh briefing that overwrites the existing cache. Outside these windows the cache is trusted, as new newsletter editions are unlikely.
